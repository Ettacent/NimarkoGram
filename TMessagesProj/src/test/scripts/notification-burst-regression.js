...
public class NotificationBurstTest {
 ...
 static class ValueAnimator extends Animator {
  static int starts;float from,target,value;boolean cancelled,ended;Consumer<ValueAnimator> update;AnimatorListenerAdapter listener;
  ...
  void addListener(AnimatorListenerAdapter l){listener=l;}Float getAnimatedValue(){return value;}void start(){starts++;}
  ...
  void step(float p){if(!cancelled&&!ended){value=from+(target-from)*p;update.accept(this);if(p==1)end();}}
 ...
 static class UserConfig {static int getActivatedAccountsCount(){return 2;}static UserConfig getInstance(int a){return new UserConfig();}Object getCurrentUser(){return null;}}
 ...
 static class Banner {
  ...
  Label title=new Label(),body=new Label(),expandedBody=new Label(),text=new Label();Object watch=new Object();Anim anim=new Anim(this);
 ...
 public static void main(String[] args){
  ...
  b.finishContent();check(b.body.text.equals("message1000")&&b.expandedBody.text.equals(b.body.text)&&b.text.alpha==1&&b.pendingMessage==null,"burst settles to latest collapsed and expanded text");
  ...
  float midAlpha=b.text.alpha;update(b,3,true);
  check(b.contentAnimator==fadeOut&&b.text.alpha==midAlpha&&b.body.text.equals("m1")&&b.pendingMessage.equals("message3"),"in-flight fade-out retains old text and coalesces latest message");
  b.slot.directResize=true;fadeOut.step(1);ValueAnimator fadeIn=b.contentAnimator;
  check(fadeIn!=null&&fadeIn!=fadeOut&&!b.contentFadeOut&&b.text.alpha==0&&!b.slot.directResize,"commit at zero alpha switches to animated content sizing and fade-in");
  ...
  fadeIn.step(.4f);float visibleAlpha=b.text.alpha;update(b,4,true);ValueAnimator replacementFade=b.contentAnimator;
  check(fadeIn.cancelled&&replacementFade!=fadeIn&&b.contentFadeOut&&Math.abs(replacementFade.from-visibleAlpha)<.001f&&b.text.alpha==visibleAlpha,"arrival during fade-in reverses from current alpha without a flash");
  fadeIn.lateUpdate(1);fadeIn.lateEnd();fadeOut.lateUpdate(0);fadeOut.lateEnd();
  check(b.contentAnimator==replacementFade&&b.text.alpha==visibleAlpha&&b.body.text.equals("message3")&&b.pendingMessage.equals("message4"),"retired fade callbacks cannot mutate alpha content or current animator");
  ...
  b.finishContent();check(b.body.text.equals("message6")&&b.expandedBody.text.equals("message6")&&b.text.alpha==1&&b.contentAnimator==null,"latest message wins after fade reversal");
  ...
  for(boolean closing:new boolean[]{false,true}){
   ...
   b.closing=closing;int beforeCancel=ValueAnimator.starts;float beforeAlpha=b.text.alpha;b.cancelContentTransition();
   ...
   check(b.text.alpha==(closing?beforeAlpha:1f),"closing cancellation preserves midfade alpha while live cancellation restores visibility");
   cancelledFade.lateUpdate(0);cancelledFade.lateEnd();
   check(b.contentAnimator==null&&b.text.alpha==(closing?beforeAlpha:1f)&&b.body.text.equals(closing?"m1":"message2"),"cancelled fade cannot resurrect text alpha or transitions");
  }
  reset();b=new Banner(100,1);show(b);frame();update(b,2,true);b.contentAnimator.step(1);b.contentAnimator.step(.25f);
  b.cancelContentTransition();check(b.body.text.equals("message2")&&b.text.alpha==1&&b.contentAnimator==null,"cancelling fade-in keeps committed text visible without pending content");
  reset();b=new Banner(100,1);show(b);frame();update(b,2,true);b.opening=true;b.finishContent();
  check(b.body.text.equals("message2")&&b.expandedBody.text.equals("message2")&&b.text.alpha==1,"opening does not block latest content fade completion");
  ...
  float retiringY=b.y,retiringOffset=b.pullOffset,retiringTextAlpha=b.text.alpha;b.scaleX=.96f;b.scaleY=.98f;
  ...
  check(b.y==retiringY&&b.pullOffset==retiringOffset&&b.scaleX==.96f&&b.scaleY==.98f&&b.text.alpha==retiringTextAlpha&&b.body.text.equals("m1"),"retirement fades parent alpha without spatial change or midfade text flash");
  retiringPull.lateUpdate(-80);retiringPull.lateEnd();retiringFade.lateUpdate(1);retiringFade.lateEnd();
  check(banner==successor&&b.y==retiringY&&b.text.alpha==retiringTextAlpha&&pullCompletions[0]==0,"retired callbacks cannot navigate move or brighten outgoing card");
