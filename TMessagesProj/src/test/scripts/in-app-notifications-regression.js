...
assert.match(notifications, /if \(handled\) mBuilder.setPriority\(NotificationCompat.PRIORITY_DEFAULT\)/);
assert.match(notifications, /handled \? NotificationManager.IMPORTANCE_DEFAULT : bannerImportance/);
...
const update = extract(notifications, 'private void showOrUpdateNotification(');
assert.match(update, /\{\s*cancelBannerDelivery\(\);\s*final int bannerDelivery = bannerDeliveryGeneration;\s*final long bannerPrivacy = bannerPrivacyRevision.get\(\);\s*final long bannerSession = [\w.]+\.session\(currentAccount\);\s*final long bannerOwner = getUserConfig\(\).getClientUserId\(\);/);
...
assert.match(update, /if \(!NimarkoInAppNotifications.isAvailable\(\)\s*\|\| bannerPrivacy != bannerPrivacyRevision.get\(\)\s*\|\| previewAllowed != NimarkoInAppNotifications.canPreview\(currentAccount, bannerDialog, bannerTopic\)\)\s*\{\s*showOrUpdateNotification\(false\);\s*return;/);
