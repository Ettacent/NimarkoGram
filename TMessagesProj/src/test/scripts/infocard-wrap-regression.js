...
public class WrapTest {
 ...
 static class ValueAnimator extends Animator {
  ...
  void setDuration(long d){duration=d;}void setInterpolator(Interpolator o){}
 ...
 static class Carousel {
  ...
  float dragProgress,releaseVelocity;boolean dragUp=true,dragging,inlineFolderStyle=true,settlingToNext,dragCrossedCard;ValueAnimator animator;
  ...
  ${method('private ValueAnimator createSettleAnimator(')}
 ...
 public static void main(String[] args){
  ...
  c.animateCommit(1);check(c.animator.duration==204);ValueAnimator first=c.animator;
  ...
  c.animateCommit(1);check(c.animator.duration==Math.round(120+140*(1-visible)));
  ...
  check(c.animator.duration==204);c.cancelAnimResume();check(c.currentIndex==0&&c.dragging&&c.animator==null);
...
try {
    ...
    assert.throws(() => run(java.replace('Math.round(120 + 140 * distance)', '330')));
