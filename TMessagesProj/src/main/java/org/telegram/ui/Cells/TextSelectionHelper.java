...
public abstract class TextSelectionHelper<Cell extends TextSelectionHelper.SelectableView> {
    ...
    private long scrollLastFrameTime;
    ...
    private Runnable scrollRunnable = new Runnable() {
        ...
        public void run() {
            if (scrolling && (parentRecyclerView != null || parentNestedScrollView != null)) {
                ...
                long frameTime = scrollLastFrameTime == 0 ? 16 : Math.max(1, Math.min(32, now - scrollLastFrameTime));
                ...
                float distance = dy * frameTime / 16f + scrollRemainder;
                dy = (int) distance;
                scrollRemainder = distance - dy;
                ...
                if (dy != 0 && parentRecyclerView != null) {
                    parentRecyclerView.scrollBy(0, scrollDown ? dy : -dy);
                }
                if (dy != 0 && parentNestedScrollView != null) {
                    parentNestedScrollView.setScrollY(parentNestedScrollView.getScrollY() + (scrollDown ? dy : -dy));
                }
                View scheduler = parentRecyclerView != null ? parentRecyclerView : parentNestedScrollView;
                scheduler.postOnAnimation(this);
            }
        }
    };

    private void cancelScrollRunnable() {
        AndroidUtilities.cancelRunOnUIThread(scrollRunnable);
        if (parentRecyclerView != null) {
            parentRecyclerView.removeCallbacks(scrollRunnable);
        }
        if (parentNestedScrollView != null) {
            parentNestedScrollView.removeCallbacks(scrollRunnable);
        }
