...
public class MessageDrawable extends Drawable {
    ...
    public boolean getTopClipGeometry(Rect sourceBounds, float[] out, boolean topNear) {
    ...
    public boolean getBottomClipGeometry(Rect sourceBounds, float[] out) {
        return getBottomClipGeometry(sourceBounds, out, isBottomNear, botButtonsBottom);
    }

    public boolean getBottomClipGeometry(Rect sourceBounds, float[] out,
                                         boolean bottomNear, boolean buttonsBottom) {
        if (sourceBounds == null || out == null || out.length < 5 || sourceBounds.isEmpty()) {
            return false;
        }
        final int padding = dp(2);
