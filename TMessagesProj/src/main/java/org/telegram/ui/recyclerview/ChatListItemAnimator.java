...
public class ChatListItemAnimator extends DefaultItemAnimator {
    ...
    public void prepareThanos(RecyclerView.ViewHolder viewHolder) {
        ...
        // ThanosEffect draws the complete View into an ARGB bitmap synchronously on the UI
        // thread. A long text message can be several screens tall, while selecting multiple
        // messages can allocate and draw several such bitmaps in the same frame. Keep the
        // effect for normal messages, but fall back to the regular fade once its snapshot work
        // exceeds one viewport (with an absolute cap for high-resolution displays).
